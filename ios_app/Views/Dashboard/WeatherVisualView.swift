...
private enum WeatherPainter {
    ...
    private static func drawSunny(_ ctx: inout GraphicsContext, progress: CGFloat) {
        ...
        for _ in 0..<8 {
            ...
            ctx.rotate(by: Angle.radians(.pi / 4))
        }
        ctx.restoreState()
    ...
    private static func drawSlightlyCloudy(_ ctx: inout GraphicsContext, progress: CGFloat) {
        ...
        ctx.saveState()
        ctx.translateBy(x: 64, y: 34 + sunHover)
        ...
        // Mini sun rays
        for i in 0..<6 {
            let angle = CGFloat(i) * (.pi / 3) + progress * (.pi * 0.5)
            let p1 = CGPoint(x: cos(angle) * 16, y: sin(angle) * 16)
            let p2 = CGPoint(x: cos(angle) * 21, y: sin(angle) * 21)
            var line = Path()
            line.move(to: p1)
            line.addLine(to: p2)
            ctx.stroke(line, with: .color(Color(hex: 0xFBBF24)), style: StrokeStyle(lineWidth: 2.5, lineCap: .round))
        }
    ...
    private static func drawVolumetricCloud(
        ...
        var path = Path()
        path.addRoundedRect(in: CGRect(x: -28, y: 2, width: 56, height: 18), cornerRadius: 9)
        ...
        if !isStorm {
            ...
            ctx.stroke(highlight, with: .linearGradient(
